...
const duplicate = `${stack}  patilu-web-extra:\n    image: ghcr.io/fer336/patilu-web:production\n`;
assert.throws(() => setStackVersion(duplicate, "v1.0.0"), /exactly one reference/);

const unknown = stack.replace("patilu-web:production", "patilu-worker:production");
...
assert.throws(() => validateReleaseLineage({ ...recovery, mainStack: updated.replace("v1.2.3", "v1.2.4") }), /exact release pin/);
